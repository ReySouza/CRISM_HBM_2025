...
def export_envi_masks(pred, im_shape, output_dir, transform=None, crs=None):
    """
    Export mineral class predictions as ENVI mask files, organized by class.
    ...
    output_dir : str
        Parent directory to save the ENVI mask files.
    transform : Affine, optional
        Affine transform for the georeferencing of the mask files.
    crs : dict, optional
        Coordinate reference system for the ENVI files.
    """
    os.makedirs(output_dir, exist_ok=True)  # Create the parent masks directory
    pred_reshaped = pred.reshape(im_shape)
    ...
    for cls in np.unique(pred):
        ...
        mask = (pred_reshaped == cls).astype(np.uint8)

        # Get the class name and create a subdirectory for the class
        class_name = FULL_NAMES.get(cls, f"class_{cls}")
        class_dir = os.path.join(output_dir, class_name)
        os.makedirs(class_dir, exist_ok=True)

        # Define the ENVI mask file base name (without .hdr extension)
        mask_filename = os.path.join(class_dir, class_name)
        ...
        meta = {
            ...
            'height': im_shape[0],
            'count': 1,
            'crs': crs or rasterio.crs.CRS({'init': 'EPSG:4326'}),
            'transform': transform or from_origin(0, 0, 1, 1),  # Default transform
        }

        # Write the mask to an ENVI file
        with rasterio.open(mask_filename, 'w', **meta) as dst:
            dst.write(mask, 1)

        logging.info("Saved ENVI mask for class '%s' to %s", class_name, mask_filename)
...
def run_on_images(images, datadir, workdir, thresholds=(0.5, 0.7), plot=False, pred=False, stat=False, mask=False):
    ...
    for im_path in images:
        ...
        if mask:
            ...
            export_envi_masks(pred, im_shape, masks_dir)
        ...
        if stat:
            ...
            for cls in unique_classes:
                ...
                cls_mask = pred == cls
                cls_confidences = pp_[cls_mask]

                # Map class number to mineral name
                mineral_name = FULL_NAMES.get(cls, f"Unknown Class {cls}")

                if mineral_name not in stats["per_class"]:
                    stats["per_class"][mineral_name] = {
                        "max_confidence": None,
                        "min_confidence": None,
                        "avg_confidence": None,
                        "std_confidence": None,
                ...
                if cls_confidences.size > 0:
                    stats["per_class"][mineral_name] = {
                        "max_confidence": float(np.max(cls_confidences)),
                        "min_confidence": float(np.min(cls_confidences)),
                        "avg_confidence": float(np.mean(cls_confidences)),
                        "std_confidence": float(np.std(cls_confidences))
                    }
